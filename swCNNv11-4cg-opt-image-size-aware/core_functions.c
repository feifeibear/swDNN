  ...
  for(RoStart = 0; RoStart < Ro; RoStart++){
    for(CoStart = 0; CoStart < Co; CoStart+=bCo){
      ...
      //DMA the RoStart+cKr line of input
  		...
  		//dma_wait(&replyget_input, 1); replyget_input = 0;
        if(cKr != K-1){
  		  	dma(dma_get_input, (long)(input_base + ((RoStart+cKr+1)*Ni*8*Ci+CoStart)*4), 
            (long)(local_input + input_load_index*local_input_size));
  		  }
  		  else{
  		  	if(CoStart+bCo < Co){
  		  		dma(dma_get_input, (long)(input_base + ((RoStart)*Ni*8*Ci+CoStart+bCo)*4), 
              (long)(local_input + input_load_index*local_input_size));
  		  	}
  		  	else{
  		  		dma(dma_get_input, (long)(input_base + ((RoStart+1)*Ni*8*Ci)*4), 
              (long)(local_input + input_load_index*local_input_size));
  		  	}
  		  }
