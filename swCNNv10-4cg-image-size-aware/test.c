 ...
 *
 * Oct.10 image layout is (4, Ci, Ni, 8, Ri)
 *
 * ****************************/
...
void naiveConv(Type*, Type*, Type*);//config of data
...
//4CG
	...
	int ldm_consume =  2*Ni/8*bCo*4 + 2*Ni*No/64 + No/8*bCo*4; 
	printf("ldm comsumption is %d KB\n", ldm_consume*8);
	assert(ldm_consume*8 < 60*1024);
  ...
  init(input, weight, output, test_output);
	...
	for(s = 0; s < TEST_STEPS; s ++){
		for(i = 0; i < NUM_CG; ++i){ 
          params[i]->input = input + i*(Ro/4)*4*Ci*Ni*8;
          params[i]->weight = weight;
          params[i]->output = output + i*(Ro/4)*4*Co*No*8;
	        params[i]->_Ni = Ni;
